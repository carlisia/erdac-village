...
- `information_schema.COLUMN_TYPE` reports `SVECTOR` without its width. Use `SHOW CREATE TABLE` for the declared dimension, and `VECTOR_DIMENSION(col)` for a stored value's actual width; `LENGTH`, `CONVERT` and `JSON_LENGTH` all refuse an `SVECTOR` with error 1221. Measured 2026-09-08.
